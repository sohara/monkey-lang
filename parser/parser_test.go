...
func TestParsingPrefixExpressions(t *testing.T) {
	...
	for _, tt := range prefixTests {
		...
		if len(program.Statements) != 1 {
			t.Fatalf("program.Satemetns does not contain %d statements. got=%d", 1, len(program.Statements))
		}
		...
		if !ok {
			t.Fatalf("stmt is not ast.PrefixExpressioni. got=%T", stmt.Expression)
		}
		...
		if !testIntgerLiteral(t, exp.Right, tt.integerValue) {
			return
		}
	}
}
